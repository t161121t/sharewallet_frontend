...
Phase 1 — Step 4：`/login` をモックアップ通りの「ログイン」画面にするための実装内容と手順をまとめます。  
...
Phase 1 の「ホーム・ログインまで」はここで一通り揃います。次は `06-Phase2実装計画.md` へ進んでください。新規登録画面（`/register`）をログイン画面をベースに入力3つ＋「新規登録」ボタンで完成させるなど、Phase 2 の実装に進みます。
