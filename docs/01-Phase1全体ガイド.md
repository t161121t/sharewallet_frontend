# 01. Phase 1 全体ガイド（ホーム・ログインまで）

モックアップに沿って「金貨アイコン → ハート型財布ロゴ → ホーム画面 → ログイン画面」の順で実装するための全体ガイドです。  
各ステップの詳細は `02`〜`05` の個別ドキュメントを参照してください。
...
## Step 1：金貨アイコン（CoinIcon）　→ 詳細: `02-CoinIcon実装手順.md`
...
## Step 2：ハート型財布ロゴ（HeartWalletLogo）　→ 詳細: `03-HeartWalletLogo実装手順.md`
...
## Step 3：ホーム画面（page.tsx）　→ 詳細: `04-ホーム画面実装手順.md`
...
## Step 4：ログイン画面の完成（login/page.tsx）　→ 詳細: `05-ログイン画面実装手順.md`
...
Phase 1 完了後は `06-Phase2実装計画.md` へ進んでください。
